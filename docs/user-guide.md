...
`POST /v1/images/generations` accepts the OpenAI-compatible `model`, `prompt`, `quality`, `size`, `n`, and `response_format` fields. Use the public virtual model `model: "codex-gpt-image-2"` and `n: 1` to route through a Codex subscription. `gpt-image-2` remains a separate standard API model routed through an OpenAI API Provider. When a `gpt-image-2` request carries a Codex `originator` or `x-codex-image-turn-id` header, TokenHub preserves that model and route while forcing a `b64_json` response; the API key must allow `gpt-image-2`. Add `Prefer: respond-async` to receive an image job, then poll `GET /v1/image-jobs/{id}`.

`POST /v1/images/edits` normally accepts up to 16 multipart reference images in `image` or `image[]`. As a narrow native-client exception, requests marked by a configured image-alias header or originator prefix may use `application/json` with Base64 data URLs in `images[].image_url`. For Codex-marked JSON edits, TokenHub preserves `gpt-image-2` and its OpenAI API route while always returning `b64_json`; each decoded image is limited to 50 MB and the whole request to 128 MB. Ordinary JSON clients still receive `415 invalid_content_type`. `gpt-image-2` forwards one multipart `mask` to the OpenAI API; mask edits are not available through Codex subscription accounts. TokenHub sends image requests directly to the selected provider without installing or starting Codex CLI, keeps prompts encrypted in the database, retains input and output images on the server, and returns signed download URLs valid for 24 hours. The files remain stored after a URL expires; polling the job creates a new URL.
